...
def main():
    ...
    # Target area: Left edge, Row 4 (usually Shift)
    # Based on previous output, Y is roughly 65-68
    print("Potential holes in Left Shift area (X < 70, Y 60-75):")
    shift_area = [r for r in rects if r[0] < 70 and 60 < r[1] < 75]
    shift_area.sort(key=lambda r: (r[1], r[0]))
    
    for i, (cx, cy, w, h) in enumerate(shift_area):
