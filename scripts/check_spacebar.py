...
def main():
    ...
    # Spacebar area roughly around X=130 (centered for 96% is offset)
    # Looking for wide rectangles in row 1 (Y ~ 10-15)
    print("Potential holes in Spacebar area (Y < 20):")
    space_area = [r for r in rects if r[1] < 20 and r[2] > 20]
    space_area.sort(key=lambda r: (r[1], r[0]))
    
    for i, (cx, cy, w, h) in enumerate(space_area):
