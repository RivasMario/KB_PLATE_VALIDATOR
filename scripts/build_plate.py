...
def emit_dxf(out_path, cutouts_polys, screw_holes=None, outline_segments=None, screw_radius=1.2):
    ...
    if outline_segments:
        # Join connected segments into a single loop
        current_loop = []
        for seg in outline_segments:
            if seg[0] == 'line':
                if not current_loop: current_loop.append(seg[1])
                current_loop.append(seg[2])
            elif seg[0] == 'arc':
                res = arc_from_3pts(seg[1], seg[2], seg[3])
                if not res:
                    if not current_loop: current_loop.append(seg[1])
                    current_loop.append(seg[3])
                else:
                    (ux, uy), r, s, e = res
                    sweep = (e - s) % 360
                    bulge = math.tan(math.radians(sweep) / 4.0)
                    # Add bulge to the last vertex in the loop
                    if not current_loop: current_loop.append(seg[1])
        ...
        if current_loop:
            msp.add_lwpolyline(current_loop, dxfattribs={'layer': L_OUTLINE})
...
def generate_svg_string(plate_w, plate_h, outline_poly, cutouts_polys, screws, screw_radius):
    # Calculate true bounds from geometry
    all_geoms = [outline_poly] + cutouts_polys
    total_union = unary_union(all_geoms)
    ...
    if isinstance(outline_poly, Polygon):
        lines.append(f'<path d="{poly_to_path(outline_poly)}" fill="#e2e8f0" stroke="#64748b" stroke-width="1"/>')
    elif isinstance(outline_poly, MultiPolygon):
        for p in outline_poly.geoms:
            lines.append(f'<path d="{poly_to_path(p)}" fill="#e2e8f0" stroke="#64748b" stroke-width="1"/>')
...
def generate_plate(kle_path=None, out_path=None, pcb_path=None,
    ...
    if pcb_path:
        ...
        if pcb_outline:
            outline_segments = [transform_segment(seg, nudge) for seg in pcb_outline]
        ...
        # Form-fitting Split Logic: Union of key bounding boxes + padding
        key_boxes = []
        for k in keys:
            cx, cy = k['cx_u'] * U1 + pad, k['cy_u'] * U1 + pad
            angle = k.get('_r_ccw', 0)
            # Create a box slightly larger than the 1u key footprint
            b = box(cx - U1/2, cy - U1/2, cx + U1/2, cy + U1/2)
            if angle: b = affinity.rotate(b, angle, origin=(cx, cy))
            key_boxes.append(b)
        
        # Merge all key footprints, buffer by padding, and simplify
        ...
        # Convert Shapely outline back to segments for emit_dxf
        def poly_to_segs(p):
